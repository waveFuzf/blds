...
public class HzSupplementReportServiceImpl implements HzSupplementReportService{
    ...
    @Override
    public Result saveSupplementReport(String specialistId, Integer sign, Integer signtype, HzSupplementReport supplementReport, String slideList, Integer supplementSlideType, Integer priceTypeId, Integer doctorPositionId) {
        int status;
    ...
    @Override
    public HzSupplementReport selectSuppleReport(String consult_id) {
        Example example=new Example(HzSupplementReport.class);
        example.createCriteria().andEqualTo("consultId",consult_id).andEqualTo("isDelete",0);
        return SupplementReportMapper.selectOneByExample(example);
    }
